  ...
  if(!missing(path)){
    ...
    if(design %in% c("natural_history", "both")){
      ...
      json_exp_design = crcexperiment_to_json(self = self,
                                              experimental_design = self$nh_design,
                                              type = "nh",
                                              write_inputs = write_inputs)

      message(paste0("Writing Natural History Experimental Design JSON File with ",
                     nrow(json_exp_design),
                     " json rows."))
    ...
    if(design %in% c("screening", "both")){
      ...
      if(format == "csv") {
        csv_exp_design = crcexperiment_to_csv(self = self,
                                               experimental_design = self$screening_design,
                                               type = "screening",
                                               write_inputs = write_inputs,
                                               block_ids = block_ids) %>%
